...
static int GetMemberID(char *id)
{
	...
	int listID;								/* 会員情報リスト内のID（下位10桁の数字） */

	/* 12桁の会員IDを作成する（INT型では10桁までしか取れないので先頭の'2'を削除する） */
	GetToday(0, 0, 0, today); 				/* 今日の日付を取得 */
	strncpy_s(w, GETMEMBERID_STR_BUF_SIZE, today + 1, 7);		/* 2012年の先頭の2を削除 */
	sprintf_s(w, GETMEMBERID_STR_BUF_SIZE, "%08s%02d", w, 1);	/* 日付+"01"の10桁を作成 */
	newID = atoi(w);
	/* 会員情報リストを検索し、新しいIDを取得する */
	target = root;
	if (target != NULL) {
		/* リストが終了するまで繰り返す */
		while (target->next != NULL) {
			...
			/* リストの会員IDの下位10ケタを取得する（INT型では10桁まで有効） */
		...
		if ((newID % 1000) == 0) {				/* 999を超えた */
			...
			sprintf_s(id, MEMBER_ID_SIZE + 1, "992%09d", newID);	/* 文字列に変換 */
			retVal = TRUE;
		}
	}
	else {
